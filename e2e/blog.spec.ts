...
test.describe('Blog post pages (/blog/...)', () => {
  test('blog post page shows title and navbar with active Writing', async ({ page }) => {
    await page.goto('/blog/reflection_on_building_systems');
    await expect(
      page.getByRole('heading', { name: 'Reflection on Building Systems', level: 1 }),
    ).toBeVisible();
  ...
  test('navigating from Writing to blog post works', async ({ page }) => {
    ...
    const link = page.getByRole('link', { name: 'Reflection on Building Systems' });
